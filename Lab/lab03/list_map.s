...
map:
    ...
    # THINK: why a0?
    ### YOUR CODE HERE ###
    lw a0, 0(s0)    # s0 is address of node
    ...
    ### YOUR CODE HERE ###
    sw a0, 0(s0)    # store square a0 to s0
...
loop:   #do...
    li  a0, 8
    jal ra, malloc      # get memory for the next node, a0 stores address
    sw  s1, 0(a0)   # node->value = i       store word s1(i) to a0
    sw  s0, 4(a0)   # node->next = last     store word s0(i) to a0+4
    add s0, a0, x0  # last = node   0, 8, 16 ...
    addi    s1, s1, 1   # i++
    addi t0, x0, 10
